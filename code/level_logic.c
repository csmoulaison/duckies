...
void pre_update_level_logic(Game* game, Stack* stack) {
    ...
    state->level_egg_exists = false;
    state->frog_exists = false;
    ...
    switch(state->level_index) {
        ...
        case 11: { 
            state->frog_index = 0;
            state->frog_exists = true;
            state->frog_pos = iv2_new(1, 2);

            platform_car_cycle_ltr(game, push_car(state, SPRITE_CAR_A, 2), 0, 4, 12);
            platform_car_cycle_ltr(game, push_car(state, SPRITE_CAR_A, 2), 8, 4, 12);
            platform_car_cycle_rtl(game, push_car(state, SPRITE_CAR_A, 2), 3, 5, 12);
            platform_car_cycle_rtl(game, push_car(state, SPRITE_CAR_A, 2), 10, 5, 12);
        } break;
